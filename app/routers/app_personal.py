...
def resumo_completo_aluno(aluno_id: int, personal: Personal = Depends(get_personal_atual), db: Session = Depends(get_db)):
    ...
    return {
        ...
        "testes": {
            ...
            "vo2max": aval.vo2max if aval else None,
            "classificacao_vo2": aval.classificacao_vo2 if aval else None,
            "classificacao_gordura": aval.classificacao_gordura if aval else None,
            "classificacao_flexao": aval.classificacao_flexao if aval else None,
            "classificacao_flexibilidade": aval.classificacao_flexibilidade if aval else None,
        ...
        "circunferencias": {
            ...
            "quadril": aval.circ_quadril if aval else None,
            "braco_d": aval.circ_braco_d_contraido if aval else None,
            "braco_e": aval.circ_braco_e_contraido if aval else None,
            "antebraco_d": aval.circ_antebraco_d if aval else None,
            "antebraco_e": aval.circ_antebraco_e if aval else None,
            "coxa_d": aval.circ_coxa_d if aval else None,
    ...
    for i, sessao_dados in enumerate(dados.sessoes):
        ...
        for j, ex in enumerate(sessao_dados.exercicios):
            ex_sessao = ExercicioSessao(
                ...
                tempo_descanso_seg=ex.get("tempo_descanso_seg") or ex.get("descanso") or 90,
            )
            db.add(ex_sessao)
